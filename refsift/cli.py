...
from refsift.models import Verdict, VerificationReport

app = typer.Typer(name="refsift", help="Citation verification: existence, URL liveness, relevance.")
console = Console()
...
def check_cmd(
    ...
    if file == "-":
        text = sys.stdin.read()
        from refsift.parse.detect import _sniff_and_parse as parse
        refs = parse(text)
    else:
...
def check_one_cmd(
    ...
    from refsift.parse.plaintext import extract_fields, detect_style, Style
